...
| [T1-05](T1-05-pending-gate-service.md) | pending-gate-service（闸门编排 + 守卫 + discard） | L1 | T1-03, T1-04 | 已完成 | 已过 |
...
**挂账（P2，非阻塞）**：① domain 测试文件 pi import 防线弱化（reorg-verify P2-2）——T1-10 落地时补 linter 级检查（如 vitest 自检改扫全目录或 eslint no-restricted-imports）；② T1-09 复核 P3：前缀键（y/n/b）不清 render 缓存致提示滞留一帧（纯显示，一行修）——T1-10 顺手修；③ T1-10 P2×2：守卫「任何工具」收窄为 write/edit、symlink/侧车盲区——第二期 coding 模式前收口；④ T1-10 P3：probe 重名坑；⑥ T1-11 P3-1：详设 diffBetweenVersions 无卡承接，登记 consciously cut（一期不需要）；⑦ T1-11 架构裁定：web 审计直写 JSONL 维持（pi _persist 无 assistant 永不落盘），第三期合并时对照复验；⑤ T1-08/09 fork 实证后裁决 `nextstep` 常量是否带点前缀（T1-01 遗留）。
